...
# Usage:
#   scripts/run_t3_audit.sh "https://nunatechnology.com/" "nuna"

URL="${1:-}"
SLUG="${2:-case}"
DATE="$(date +%F)"
OUT_DIR="/home/bobby/.openclaw/workspace/tmp/reports/t3-${DATE}-${SLUG}"

mkdir -p "$OUT_DIR"

echo "[1] Crawl"
python3 scripts/crawl_product_info.py --url "$URL" --pretty > "$OUT_DIR/01-level0-extracts.md" || true
...
if [[ -f "$OUT_DIR/auditor_reports.json" ]]; then
  python3 scripts/synthesize_results.py --input "$OUT_DIR/auditor_reports.json" --output "$OUT_DIR/05-synthesis.json" --text
else
